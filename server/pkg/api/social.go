...
func (h *SocialHandler) AlbumFeed(c *gin.Context) {
	collectionID, err := strconv.ParseInt(c.Query("collectionID"), 10, 64)
	if err != nil || collectionID <= 0 {
		handler.Error(c, stacktrace.Propagate(ente.ErrBadRequest, "invalid collectionID"))
		return
	...
	comments, reactions, hasMoreComments, hasMoreReactions, err := h.Controller.AlbumFeed(c, userID, social.AlbumFeedRequest{
		CollectionID: collectionID,
		Since:        since,
		Limit:        limit,
	})
