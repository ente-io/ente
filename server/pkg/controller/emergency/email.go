...
const (
	...
	RecoveryCompletedTemplate       string = "legacy/recovery_completed_trusted.html"
...
func (c *Controller) createRecoveryEmailData(legacyUser, trustedUser ente.User, newStatus ente.RecoveryStatus) ([]emailData, error) {
	...
	case ente.RecoveryStatusInitiated:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("CRITICAL: %s has initiated recovery process for your account", trustedUser.Email),
	...
	case ente.RecoveryStatusRecovered:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("Your account has been successfully recovered by %s", trustedUser.Email),
			templateName: RecoveryCompletedTemplate,
			emailTo:      legacyUser.Email,
			templateData: templateData,
			inlineImages: []map[string]interface{}{inlineImage},
		})
	...
	case ente.RecoveryStatusStopped:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("%s has cancelled recovery process", trustedUser.Email),
	...
	case ente.RecoveryStatusRejected:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("%s has declined your recovery attempt", legacyUser.Email),
	...
	case ente.RecoveryStatusWaiting:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("%s is recovering your account!", trustedUser.Email),
	...
	case ente.RecoveryStatusReady:
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("You can now change password for %s", legacyUser.Email),
		...
		emailDatas = append(emailDatas, emailData{
			title:        fmt.Sprintf("%s can now change password for your account", trustedUser.Email),
