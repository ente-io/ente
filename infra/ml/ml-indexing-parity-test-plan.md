...
|---|---|---|---|
| 2026-02-16 | Codex (GPT-5) | Mobile platform runners are still missing (`mobile/apps/photos/integration_test/ml_parity_android_test.dart`, `mobile/apps/photos/integration_test/ml_parity_ios_test.dart`), so parity compare can still run with desktop/python only. | Implement W5/W6 and wire both runners into `run_suite.sh` with shared-schema outputs. |
