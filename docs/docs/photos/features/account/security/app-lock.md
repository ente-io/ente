---
title: App lock
description: Add an extra privacy layer to Ente Photos with app lock
---
...
If your device has no passcode or screen lock configured, set that up first in
system settings.
...
- **Device lock**: Use Face ID, Touch ID, or your device passcode/password.
- **PIN lock**: Set a custom PIN for Ente Photos.
- **Password lock**: Set a custom password for Ente Photos.

On desktop, **Device lock** is currently available only on macOS.

## Auto lock

Configure **Auto lock** to choose when Ente Photos locks after being put in the
background:
