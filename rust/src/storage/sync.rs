...
use crate::{Result, models::{file::RemoteFile, collection::Collection}};
...
impl<'a> SyncStore<'a> {
    ...
    pub fn get_collections(&self, account_id: i64) -> Result<Vec<Collection>> {
        ...
        let collections = stmt.query_map(params![account_id], |row| {
    ...
    pub fn get_files_by_collection(&self, account_id: i64, collection_id: i64) -> Result<Vec<RemoteFile>> {
        let mut stmt = self.conn.prepare(
        ...
        let files = stmt.query_map(params![account_id, collection_id], |row| {
    ...
    /// Update sync state
    pub fn update_sync_state(&self, account_id: i64, sync_type: &str, timestamp: i64) -> Result<()> {
    ...
    pub fn get_last_sync(&self, account_id: i64, sync_type: &str) -> Result<Option<i64>> {
        ...
        let timestamp = stmt.query_row(params![account_id], |row| {
    ...
    pub fn mark_album_file_synced(&self, account_id: i64, album_id: i64, file_id: i64) -> Result<()> {
        self.conn.execute(
