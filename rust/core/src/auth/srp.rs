...
mod tests {
    ...
    fn test_public_a_padding() {
        ...
        let session = SrpSession {
            inner: ClientG4096::<SrpSha256>::new(),
    ...
    fn test_compute_m1_and_verify_m2_match_expected() {
        ...
        let verifier_client = ClientG4096::<SrpSha256>::new();
    ...
    fn test_known_vector_leading_zero_a_matches_go_server_m1() {
        ...
        let client = ClientG4096::<SrpSha256>::new();
        let verifier = client.compute_verifier(srp_user_id.as_bytes(), &login_key, &srp_salt);
        let server = srp::ServerG4096::<SrpSha256>::new();
        ...
        let legacy_k = srp::utils::compute_k::<SrpSha256>(&g);
        let identity_hash =
            ClientG4096::<SrpSha256>::compute_identity_hash(srp_user_id.as_bytes(), &login_key);
        let legacy_x = ClientG4096::<SrpSha256>::compute_x(identity_hash.as_slice(), &srp_salt);
        let legacy_u = srp::utils::compute_u::<SrpSha256>(&session.a_public, &server_b);
