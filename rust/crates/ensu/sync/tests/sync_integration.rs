...
fn push_only_needs_sync_messages() {
    ...
    db.mark_message_synced(message_one.uuid)
        .expect("mark synced");
    let sync_state = SyncStateDb::open_sqlite_with_defaults(env.attachments_db_path.clone())
        .expect("sync state");
    sync_state
        .set_message_state(message_one.uuid, Some("remote-message-one"), Some(9))
