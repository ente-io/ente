...
let _worker: ComlinkWorker<typeof DedicatedHEICConvertWorker> | undefined;

/** Lazily created, cached, instance of our web worker. */
const worker = async () => {
    let worker = _worker;
    if (!worker) _worker = worker = createHEICConvertComlinkWorker();
    return await worker.remote;
};
