...
const installPerformanceMeasureGuard = () => {
    if (typeof window === "undefined") return;
    if (process.env.NODE_ENV !== "development") return;

    const perf = window.performance;
    if (typeof perf.measure !== "function") return;
    if ((perf as { __enteMeasureGuard?: boolean }).__enteMeasureGuard) return;
    ...
    // Dev-only: avoid crashing when React's user timing clones huge props.
    const createFallbackMeasure = (name: string): PerformanceMeasure =>
        ({
            name,
            entryType: "measure",
            startTime: 0,
    ...
    const guardedMeasure: Performance["measure"] = (
        ...
        try {
            ...
            if (
                ...
                try {
                    return originalMeasure(name, { start, end, duration });
                } catch {
                    try {
                        return originalMeasure(name);
                    } catch {
                        return createFallbackMeasure(name);
                    }
                }
            }
    ...
    try {
        ...
        (perf as { __enteMeasureGuard?: boolean }).__enteMeasureGuard = true;
    } catch {
