...
import { ItemCard, LargeFileTileOverlay } from "ente-new/photos/components/Tiles";
import { computeThumbnailGridLayoutParams, type ThumbnailGridLayoutParams } from "ente-new/photos/components/utils/thumbnail-grid-layout";
import {
...
const Page: React.FC = () => {
    ...
    const [state, dispatch] = useReducer(largeFilesReducer, initialState);
...
const initialState: LargeFilesState = {
...
const largeFilesReducer: React.Reducer<LargeFilesState, LargeFilesAction> = (
    state,
    action,
) => {
    switch (action.type) {
    ...
    <Stack sx={{ flex: 1 }}>
        <Box sx={{ flex: 1, overflow: "hidden", paddingBlockEnd: 1 }}>
            <Autosizer>
                {({ width, height }) => (
                    <LargeFilesGrid
                        {...{ width, height, largeFiles, onToggleSelection }}
                    />
                )}
            </Autosizer>
        </Box>
...
const GridRow: React.FC<ListChildComponentProps<LargeFilesGridItemData>> = memo(
    ({ index: rowIndex, style, data }) => {
        const { largeFiles, onToggleSelection, layoutParams } = data;
        const columns = layoutParams.columns;
...
const GridItem: React.FC<GridItemProps> = memo(({ item, onToggle }) => {
