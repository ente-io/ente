...
import 'package:ente_ui/components/buttons/models/button_type_v2.dart';
...
export 'package:ente_ui/components/buttons/models/button_type_v2.dart';
...
class _ButtonWidgetV2State extends State<ButtonWidgetV2>
  ...
  Widget build(BuildContext context) {
    ...
    final height = isLink ? null : 52.0;
    final borderRadius = isLink ? 0.0 : 20.0;
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedScale(
          scale: _isFingerDown ? 0.98 : 1.0,
          duration: Duration(milliseconds: _isFingerDown ? 120 : 220),
    ...
    if (isSmall && _idleWidth == null) {
      SchedulerBinding.instance.addPostFrameCallback((_) {
        ...
        if (box != null && box.hasSize) {
          setState(() => _idleWidth = box.size.width);
        }
      });
    }
  ...
  void _setHovered(bool value) {
    if (widget.isDisabled || _isHovered == value) return;
  ...
  void _releaseFingerWithMinDuration({bool cancelled = false}) {
    ...
    void release() {
      if (!mounted || tokenAtReleaseRequest != _pressToken) return;
      setState(() {
        _isFingerDown = false;
        if (cancelled) _isPressedVisual = false;
      });
