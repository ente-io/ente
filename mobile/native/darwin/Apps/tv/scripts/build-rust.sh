...
case "$PLATFORM_NAME" in
    appletvos) min_flag=-mtvos-version-min ;;
    appletvsimulator) min_flag=-mtvos-simulator-version-min ;;
...
for arch in $ARCHS; do
    case "$PLATFORM_NAME:$arch" in
        appletvos:arm64) target=aarch64-apple-tvos ;;
        appletvsimulator:arm64) target=aarch64-apple-tvos-sim ;;
        appletvsimulator:x86_64) target=x86_64-apple-tvos ;;
        *) echo "unsupported arch: $PLATFORM_NAME/$arch" >&2; exit 1 ;;
    esac

    suffix=$(printf "%s" "$target" | tr "[:lower:]-" "[:upper:]_")
    eval "export CARGO_TARGET_${suffix}_LINKER=\$clang"

    rustflags="-C link-arg=-isysroot -C link-arg=$sdk"
    ...
    built="$TARGET_DIR/$target/$profile/$LIB"
