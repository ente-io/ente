...
struct Provider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> FileEntry {
        FileEntry(date: Date(), index: nil, iconPath: nil)
    }

    func getSnapshot(in context: Context, completion: @escaping (FileEntry) -> Void) {
        let entry = FileEntry(date: Date(), index: nil, iconPath: nil)
        completion(entry)
    }

    // Function to convert image file at URL to base64 string
    func base64StringFromImage(at fileURL: URL) -> String? {
        do {
            let imageData = try Data(contentsOf: fileURL)  // Load image data
            let base64String = imageData.base64EncodedString(options: .lineLength64Characters)  // Convert to Base64
            return base64String
        } catch {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> Void) {
        ...
        if totalSet != nil && totalSet! > 0 && prefixPath != nil {
            totalSet = 1 ?? (totalSet! > 5 ? 5 : totalSet!)
            for offset in 0..<totalSet! {
                let randomInt = Int.random(in: 0..<totalSet!)
                let entryDate = Calendar.current.date(
                    byAdding: .minute, value: X * offset, to: currentDate)!
                var iconPath =
                    data?.string(forKey: "memory_" + String(randomInt))
                let entry = FileEntry(date: entryDate, index: randomInt, iconPath: iconPath)
                entries.append(entry)
            }
        } else {
            let entry = FileEntry(date: Date(), index: -1, iconPath: nil)
            entries.append(entry)
        }
...
struct SlideshowWidgetEntryView: View {
    ...
    var body: some View {
        VStack.init(
            ...
            content: {
                if entry.iconPath != nil {
                    Image(uiImage: UIImage(contentsOfFile: entry.iconPath!)!)
                        .resizable()
                        .scaledToFill()
                    // Text(entry.iconPath!)
                } else {
...
#Preview(as: .systemSmall) {
    ...
    FileEntry(date: .now, index: nil, iconPath: nil)
    FileEntry(date: .now, index: nil, iconPath: nil)
}
