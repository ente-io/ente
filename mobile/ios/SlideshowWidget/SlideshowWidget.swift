...
struct SlideshowWidgetEntryView: View {
    ...
    init(entry: Provider.Entry) {
        self.entry = entry
        var dictionary = data?.dictionary(forKey: "slideshow_\(entry.index)_data")
        generatedId = dictionary?["generatedId"] as? String
        ...
        GeometryReader { geometry in
                ...
                let uiImage = UIImage(contentsOfFile: imageData)
            {
                Image(uiImage: uiImage)
                    ...
                    .clipped()
                    .widgetURL(
                        URL(string: "memoryWidget://message?generatedId=\(generatedId)&homeWidget"))
