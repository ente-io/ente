...
`lib/` houses the Flutter client (`core/`, `services/`, `ui/`, `db/`). Tests live in `test/` (unit & widget) and `integration_test/`. Platform shells are `android/` and `ios/`; the Photos Rust bridge crate lives at repo-root `rust/bindings/frb/photos/`, with local build glue in `rust_builder/`. Assets, fonts, and localization configs are in `assets/`, `fonts/`, `l10n.yaml`, with generated code in `lib/generated/`. Automation scripts live under `scripts/`.

## Build, Test & Development Commands
Run `flutter pub get` after dependency edits. Launch the app with `flutter run --flavor independent`. Regenerate Rust bindings from `rust/` with `cargo codegen frb` before builds. Android releases rely on `flutter build apk --release --flavor independent`; for iOS run `cd ios && pod install` then `flutter build ios`.
...
1. Install Flutter `3.38.10` with a Dart SDK between `>=3.10.0 <4.0.0`.
2. Install Rust.
3. Regenerate Rust bindings whenever native APIs shift from `rust/` using `cargo codegen frb`.
